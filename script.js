...
let hasRecordedResult = false; // 二重記録防止フラグ
...
let isOnlineGameActive = false; // 対局進行中フラグ
...
function checkAndMigrateRatings() {
  ...
  if (!isMigrated) {
    ...
    records.forEach(r => r.rating = 100);
...
function saveProfileName() {
  ...
  if (newName) {
    ...
    let records = getRankingRecords();
    const existing = records.find(r => r.name === oldName || r.name === newName);
    if (existing) {
      existing.name = newName;
      localStorage.setItem('othello_rankings_data', JSON.stringify(records));
    } else {
      updatePlayerRatingRecord(newName, getUserRating());
    }
...
function handleOpponentForfeit() {
  ...
  // 不戦勝（連勝カウントアップ＆レーティング上昇）
  recordGameResult(true);

  const myChange = calculateRatingChange(getUserRating(), opponentRating, 1);
  const oppChange = calculateRatingChange(opponentRating, getUserRating(), 0);
