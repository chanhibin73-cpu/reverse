...
// オンライン通信用変数
let peer = null;
let conn = null; // 対戦相手との接続 (またはホストとの接続)
let spectatorConns = []; // ホスト時に観戦者へ盤面をマルチキャストするための配列
...
// --- レーティングシステム（将棋 Elo Rating / 初期値: 0） ---
...
function startRandomMatch() {
  ...
  // 1. まずロビー（LOBBY_PEER_ID）へ接続を試みる（既存の待機者がいるかチェック）
  peer = new Peer();
  peer.on('open', () => {
    ...
    lobbyConn.on('open', () => {
      ...
      myRole = WHITE; // 待機者に接続できた場合は後攻（白）
      conn = lobbyConn;
      setupConnection();
      conn.send({ type: 'JOIN_RANDOM', rating: getUserRating() });
    });

    // 2. ロビーが存在しない＝自分が最初の待機者。ロビーIDで部屋を作成
    setTimeout(() => {
      if (!connectedToLobby) {
        lobbyConn.close();
        if (peer) peer.destroy();
    ...
    c.on('data', (data) => {
      if (data.type === 'JOIN_REQUEST') {
        if (!conn) {
          ...
          c.send({ type: 'JOIN_ACCEPT', role: WHITE, board: board, turn: currentPlayer, rating: getUserRating() });
          setupConnection();
          startGame('online');
...
function joinRoom() {
  ...
  peer.on('open', () => {
    ...
    targetConn.on('open', () => {
      conn = targetConn;
      conn.send({ type: 'JOIN_REQUEST', rating: getUserRating() });
    });
...
function handleNetworkMessage(data, fromConn) {
  if (data.type === 'JOIN_RANDOM') {
    ...
    fromConn.send({ type: 'START_RANDOM', rating: getUserRating() });
  ...
  if (isGameOver) {
    ...
    if (blackCount > whiteCount) {
      ...
      if (gameMode === 'online' && myRole !== SPECTATOR) {
        ...
        outcomeText = ` (レーティング: ${change >= 0 ? '+' : ''}${change})`;
      }
      ...
      if (gameMode === 'online' && myRole !== SPECTATOR) {
        const change = calculateRatingChange(getUserRating(), opponentRating, 0.5);
        setUserRating(getUserRating() + change);
        outcomeText = ` (レーティング: ${change >= 0 ? '+' : ''}${change})`;
      }
    }
