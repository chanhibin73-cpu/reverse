...
function updateAIAnalysis() {
  setTimeout(() => {
    ...
    // サクサク動かすため基本4手読み。終盤は6手読み。
    let depth = emptyCells <= 12 ? 6 : 4; 
    
    // スコア計算 (minimaxは白目線での評価を返す)
    ...
    // -100〜100のスコアを0%〜100%のバー幅に変換
    let percentage = Math.round((1 / (1 + Math.exp(-blackScore / 100))) * 100);
