// --- Firebase SDK インポート (ブラウザ直接読み込み用 CDN) ---
...
async function initFirebaseUser() {
  try {
    ...
    // Firestore上にプレイヤーデータが存在するか確認
    const userRef = doc(db, "users", currentUser.uid);
    const snap = await getDoc(userRef);

    if (snap.exists()) {
      const data = snap.data();
      if (data.rating !== undefined) localStorage.setItem('othello_user_rating', data.rating);
      if (data.displayName) localStorage.setItem('othello_player_name', data.displayName);
    } else {
      // 初期データの作成
    ...
    console.error("Firebaseログイン・データ読み込みエラー:", e);
  }
}

// --- Firebase への戦績・レーティング更新 ---
async function syncUserStatsToFirebase(newRating, resultType) {
  ...
  try {
    ...
    else if (resultType === 'loss') losses++;
    ...
    console.error("Firebaseへの戦績同期に失敗:", e);
  }
}

// --- Firestoreから全国ランキングを取得 ---
async function loadRanking() {
  const listElem = document.getElementById('ranking-list');
  if (!listElem) return;
  listElem.innerHTML = '<div style="opacity: 0.6; padding: 10px;">ランキングを読み込み中...</div>';
  ...
  try {
    ...
    if (querySnapshot.empty) {
      listElem.innerHTML = '<div style="opacity: 0.6; padding: 10px;">ランキングデータがありません</div>';
      return;
    }
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      item.innerHTML = `
        <div class="player-info">
          ...
          <span>${escapeHtml(player.displayName || '名無し')} ${isSelf ? '(あなた)' : ''}</span>
        </div>
        <div class="player-rating">${player.rating || 100} R</div>
      `;
    ...
    listElem.innerHTML = '<div style="opacity: 0.6; padding: 10px;">ランキングの取得に失敗しました</div>';
  }
}
...
// --- 初期データ移行 & プロフィール・レーティング・連勝記録 ---

function checkAndMigrateRatings() {
  const isMigrated = localStorage.getItem('othello_v100_migrated');
  if (!isMigrated) {
    setUserRating(100);
    localStorage.setItem('othello_v100_migrated', 'true');
  }
}
...
async function saveProfileName() {
  ...
  if (newName) {
    localStorage.setItem('othello_player_name', newName);
    ...
    if (currentUser) {
      try {
        ...
        console.error("Firebase名前更新エラー:", e);
      }
    }
...
function recordGameResult(isWin) {
  ...
  updateProfileStreakUI();
}

function updateProfileStreakUI() {
  const currentElem = document.getElementById('current-streak-val');
  const maxElem = document.getElementById('max-streak-val');
  if (currentElem) currentElem.innerText = getCurrentStreak();
  if (maxElem) maxElem.innerText = getMaxStreak();
}

function updateRatingUI() {
  const val = getUserRating();
  const elem = document.getElementById('user-rating-val');
  const profElem = document.getElementById('profile-rating-val');
  if (elem) elem.innerText = val;
  if (profElem) profElem.innerText = val;
}
...
function startGame(mode, level = 1) {
  gameMode = mode;
  aiLevel = level;
  isOnlineGameActive = (mode === 'online' && myRole !== SPECTATOR);
...
function createRoom() {
  ...
  peer.on('error', (err) => {
    if (err.type === 'unavailable-id') {
      alert("その部屋はすでに存在します。「部屋に参加」を押してください。");
    }
    cancelOnlineWaiting();
  });
}
...
function setupConnection() {
  ...
  conn.on('close', () => {
    if (gameMode === 'online' && isOnlineGameActive && myRole !== SPECTATOR) {
...
function leaveOnlineAndReturnMenu() {
  if (gameMode === 'online' && isOnlineGameActive && myRole !== SPECTATOR) {
    if (!confirm("対局を途中で破棄すると【不戦敗】となり、レーティングが減少します。本当に退出しますか？")) {
      return;
    }
    isOnlineGameActive = false;
    if (conn && conn.open) {
    ...
    const newRating = getUserRating() + change;
    setUserRating(newRating);
    
    // Firebaseに不戦敗を反映
    syncUserStatsToFirebase(newRating, 'loss');

    alert(`対局を破棄しました。（レーティング ${change} R）`);
  }
...
function handleOpponentForfeit() {
  ...
  const myChange = calculateRatingChange(getUserRating(), opponentRating, 1);
  const newRating = getUserRating() + myChange;
  setUserRating(newRating);

  // Firebaseに不戦勝を反映
  syncUserStatsToFirebase(newRating, 'win');

  playSE('win');
  alert(`対戦相手が退出・切断したため【不戦勝】となりました！\n（レーティング +${myChange} R）`);
  
  const turnDisplay = document.getElementById('turn-display');
  if (turnDisplay) turnDisplay.innerText = "不戦勝！";
  showBanner(`不戦勝！ (+${myChange} R)`, 4000);
}
...
function updateStatus(isGameOver = false) {
  ...
  if (isGameOver) {
    ...
    if (gameMode === 'online' && myRole !== SPECTATOR) {
      isUserWinner = (blackCount > whiteCount && myRole === BLACK) || (whiteCount > blackCount && myRole === WHITE);
    } else {
      isUserWinner = (blackCount > whiteCount);
    }
    ...
    if (blackCount > whiteCount) {
      ...
      if (gameMode === 'online' && myRole !== SPECTATOR) {
        ...
        setUserRating(newRating);
        syncUserStatsToFirebase(newRating, result === 1 ? 'win' : 'loss');
      ...
      if (gameMode === 'online' && myRole !== SPECTATOR) {
        ...
        setUserRating(newRating);
        syncUserStatsToFirebase(newRating, 'draw');
    ...
    if (myRole !== SPECTATOR) {
      recordGameResult(isUserWinner && (blackCount !== whiteCount));
    }
...
window.addEventListener('DOMContentLoaded', async () => {
  checkAndMigrateRatings();
  updateRatingUI();
  updateProfileStreakUI();
  await initFirebaseUser();
});

// HTML（onclick属性）から呼び出せるように window オブジェクトに登録
...
window.cancelOnlineWaiting = cancelOnlineWaiting;
window.leaveOnlineAndReturnMenu = leaveOnlineAndReturnMenu;
