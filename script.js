...
let isOnlineGameActive = false; // オンライン対戦進行中フラグ
...
// ルーム・ロビー設定
const ROOM_PREFIX = "othello-app-room-v3-";
const LOBBY_PEER_ID = "othello-app-random-lobby-v3";
...
// --- ユーザーデータ管理（初期レーティング 100） ---
function getUserName() {
  return localStorage.getItem('othello_user_name') || "プレイヤー";
}

function setUserName(name) {
  const trimmed = name.trim() || "プレイヤー";
  localStorage.setItem('othello_user_name', trimmed);
  updateProfileUI();
}

function getUserRating() {
  const saved = localStorage.getItem('othello_user_rating');
  // 未保存または0の場合、初期レーティング100に補正
  if (!saved || parseInt(saved, 10) === 0) {
    localStorage.setItem('othello_user_rating', 100);
    return 100;
  }
...
function setUserRating(newRating) {
  localStorage.setItem('othello_user_rating', Math.max(0, newRating));
  updateProfileUI();
}

function getCurrentStreak() {
  const saved = localStorage.getItem('othello_current_streak');
  return saved ? parseInt(saved, 10) : 0;
}

function getMaxStreak() {
  const saved = localStorage.getItem('othello_max_streak');
  return saved ? parseInt(saved, 10) : 0;
}

function updateStreakOnGameEnd(isWin, isDraw) {
  if (isDraw) return;
  ...
  localStorage.setItem('othello_current_streak', current);
  localStorage.setItem('othello_max_streak', max);
  updateProfileUI();
}
...
function updateProfileUI() {
  ...
  if (document.getElementById('menu-user-rating')) document.getElementById('menu-user-rating').innerText = rating;
  if (document.getElementById('username-input')) document.getElementById('username-input').value = name;
  if (document.getElementById('prof-rating')) document.getElementById('prof-rating').innerText = `${rating} R`;
  if (document.getElementById('prof-current-streak')) document.getElementById('prof-current-streak').innerText = `${currentStreak} 連勝`;
  if (document.getElementById('prof-max-streak')) document.getElementById('prof-max-streak').innerText = `${maxStreak} 連勝`;
}

function saveUserNameFromInput() {
  const input = document.getElementById('username-input').value;
  setUserName(input);
  showBanner("ユーザー名を保存しました！");
}

// 将棋風イロレーティング計算 (K=32)
function calculateRatingChange(myRating, oppRating, result) { // result: 1=勝, 0.5=引分, 0=負
...
function updateMatchHeaderUI(spectatorBlackInfo = null, spectatorWhiteInfo = null) {
  const blackNameElem = document.getElementById('black-player-name');
  const blackRatingElem = document.getElementById('black-player-rating');
  const whiteNameElem = document.getElementById('white-player-name');
  const whiteRatingElem = document.getElementById('white-player-rating');

  if (myRole === SPECTATOR && spectatorBlackInfo && spectatorWhiteInfo) {
    blackNameElem.innerText = spectatorBlackInfo.name;
    blackRatingElem.innerText = `R:${spectatorBlackInfo.rating}`;
    whiteNameElem.innerText = spectatorWhiteInfo.name;
    whiteRatingElem.innerText = `R:${spectatorWhiteInfo.rating}`;
    return;
  }
  ...
  if (myRole === BLACK) {
    blackNameElem.innerText = myName;
    blackRatingElem.innerText = `R:${myRating}`;
    whiteNameElem.innerText = opponentName;
    whiteRatingElem.innerText = `R:${opponentRating}`;
  } else if (myRole === WHITE) {
    whiteNameElem.innerText = myName;
    whiteRatingElem.innerText = `R:${myRating}`;
    blackNameElem.innerText = opponentName;
    blackRatingElem.innerText = `R:${opponentRating}`;
  }
}
    ...
    c.on('data', (data) => {
      if (data.type === 'JOIN_REQUEST') {
        if (!conn) {
          ...
          setupConnection();
          startGame('online');
          showBanner(`${opponentName} が接続しました！`);
        } else {
          ...
          c.send({ 
            ...
            board: board, 
            turn: currentPlayer,
            blackInfo: { name: getUserName(), rating: getUserRating() },
            whiteInfo: { name: opponentName, rating: opponentRating }
          });
        }
      } else {
  ...
  peer.on('open', () => {
    ...
    targetConn.on('data', (data) => {
      if (data.type === 'JOIN_ACCEPT') {
        ...
        setupConnection();
        startGame('online');
        showBanner(`${opponentName} の部屋に参加しました！`);
        ...
        setupConnection();
        startGame('online');
        updateMatchHeaderUI(data.blackInfo, data.whiteInfo);
...
function setupConnection() {
  ...
  conn.on('close', () => {
    // 進行中の場合、相手の途中退出（不戦勝）として処理
    if (isOnlineGameActive && myRole !== SPECTATOR) {
...
function confirmLeaveOnline() {
  if (isOnlineGameActive && myRole !== SPECTATOR) {
    if (confirm("対戦途中で退出すると【降参扱い】となり、レーティングが下がり連勝もリセットされます。本当に退出しますか？")) {
      // 相手へ通知
      if (conn && conn.open) {
        conn.send({ type: 'SURRENDER' });
      }
      // ペナルティ（敗北扱い）処理
      const change = calculateRatingChange(getUserRating(), opponentRating, 0);
      setUserRating(getUserRating() + change);
      updateStreakOnGameEnd(false, false);
      showBanner(`降参しました (R: ${change})`);
...
// 相手が途中破棄した場合の不戦勝処理
function handleOpponentAbandonment() {
  if (!isOnlineGameActive) return;
  isOnlineGameActive = false;

  const change = calculateRatingChange(getUserRating(), opponentRating, 1);
  setUserRating(getUserRating() + change);
  updateStreakOnGameEnd(true, false);
  
  playSE('win');
  showBanner(`相手が途中破棄しました！不戦勝！(R: +${change})`, 4000);

  const turnDisplay = document.getElementById('turn-display');
  turnDisplay.innerText = '相手の破棄により不戦勝！';
...
function handleNetworkMessage(data, fromConn) {
  if (data.type === 'JOIN_RANDOM') {
    ...
    document.getElementById('cancel-online-btn').style.display = 'none';
    startGame('online');
    showBanner(`${opponentName} とマッチングしました！`);
    fromConn.send({ 
      type: 'START_RANDOM', 
      name: getUserName(), 
      rating: getUserRating() 
...
function updateStatus(isGameOver = false) {
  ...
  if (isGameOver) {
    isOnlineGameActive = false; // 正常終了
    analysisTitle.innerText = "📊 最終結果";
    let outcomeText = "";

    const isBlackWin = blackCount > whiteCount;
    const isWhiteWin = whiteCount > blackCount;
    ...
    // オンライン対戦時のみレーティングと連勝数を処理
    if (gameMode === 'online' && myRole !== SPECTATOR) {
      const myWin = (myRole === BLACK && isBlackWin) || (myRole === WHITE && isWhiteWin);
      const result = myWin ? 1 : (isDraw ? 0.5 : 0);
      
      const change = calculateRatingChange(getUserRating(), opponentRating, result);
      setUserRating(getUserRating() + change);
      updateStreakOnGameEnd(myWin, isDraw);

      outcomeText = ` (R: ${change >= 0 ? '+' : ''}${change})`;
    }
